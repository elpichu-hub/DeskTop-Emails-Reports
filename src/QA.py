from email_config import team_lead_edgar_signature, team_lead_elite_signature
...
def send_qa_to_work_email(file_path_1, file_path_2, date_range, email_address, signature=team_lead_elite_signature):
    import pandas as pd
    import chardet
    ...
    # df_temp = pd.read_csv(file_path_1, header=None)
    ...
    df_filtered = df_columns_needed
    
    # Read the second Excel file
    df_agents = pd.read_csv(file_path_2)

    # Rename 'InternalId' column in df_agents to 'user' to match with df_filtered
    df_agents.rename(columns={'InternalId': 'escalationAgent'}, inplace=True)

    df_filtered = df_filtered.copy()
    df_filtered['escalationAgent'] = df_filtered['escalationAgent'].astype(str)

    df_agents = df_agents.copy()
    df_agents['escalationAgent'] = df_agents['escalationAgent'].astype(str)

    # Merge the two dataframes
    df_filtered = pd.merge(df_filtered, df_agents[['escalationAgent', 'FullName']], on='escalationAgent', how='left')

    # Calculate the average score for all calls
    average_score = df_filtered['totalScore'].mean()

    # Calculate the average score for each user
    average_scores = df_filtered.groupby('escalationAgent')['totalScore'].mean()
    ...
    for user in df_filtered['escalationAgent'].unique():
        user_data = df_filtered[df_filtered['escalationAgent'] == user]
        ...
        for _, call in calls_with_comments.iterrows():
            ...
            if prev_date_time != date_time:
                if total_score >= 85 and total_score < 100:
                    ...
                    result_string += f"<h3 style='text-decoration:underline;'>\n- {media} {call_number} Recorded on {date_time} Scored <span style='color: red;'>{total_score}%</span></h3>"
                call_number += 1
        ...
        send_email(subject, email_address, body, image_path, image_path_100)
