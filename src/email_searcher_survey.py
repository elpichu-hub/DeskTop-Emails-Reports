    ...
    if email_id not in processed_ids:
        ...
        if result == 'OK':
            ...
            if message_data[0] is not None and message_data[0][1] is not None:
                ...
                # Extract the agent names from the subject
                agents_for_survey = None
                agents_match = re.search(r'agents=([\w,\s]+)', email_subject, re.IGNORECASE)
                if agents_match:
                    # Get the matched string, split it into a list, and capitalize the first letter of each name
                    agents_string = agents_match.group(1)
                    agents_for_survey = [name.strip().capitalize() for name in agents_string.split(',') if name.strip()]
                    logging.info("Agents: %s", ', '.join(agents_for_survey))
                    print("Agents: %s", ', '.join(agents_for_survey))
                else:
                    agents_for_survey = None
                ...
                if file_paths:
                    try:
                        ...
                        print('sent Survey Successfully')
                        send_email_function.send_email(f'Success Survey', 'lazaro.gonzalez@conduent.com', f'Survey Sent Succesfully at {datetime.now()}')
                        ...
                        send_email_function.send_email(f'Error Survey {email_address}', 'lazaro.gonzalez@conduent.com', error_message_with_traceback)
                        logging.error('Failed to send Survey email.', exc_info=True)
                else:
