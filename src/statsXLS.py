...
# will generate daily, weekly, monthly and yearly stats given a .xlsx or xls file
def run_stats(file_path, team=None):
    ...
    if stats_type == 'Monthly':
        work_sheet['N31'].value = f'Monthly Stats {start_date_formatted} - {end_date_formatted}'
    ...
    # color top 5 aht based on resutls
    if team in email_config.escalations_variations:
        print('if')
        for cell in work_sheet['R']:
            try:
                time_obj = datetime.strptime(str(cell.value), "%H:%M:%S").time()
                # if 10:30 or less green
                if isinstance(time_obj, time) and time_obj < time(hour=0, minute=10, second=30):
                    cell.fill = green
                elif isinstance(time_obj, time) and time_obj >= time(hour=0, minute=10, second=30) and time_obj < time(hour=0, minute=11, second=00):
                    cell.fill = orange
                else: 
                    cell.fill = red
            except ValueError:
        ...
        for cell in work_sheet['R']:
            try:
                ...
                if isinstance(time_obj, time) and time_obj < time(hour=0, minute=7, second=30):
                    print('green')
                    cell.fill = green
                elif isinstance(time_obj, time) and time_obj >= time(hour=0, minute=7, second=30) and time_obj < time(hour=0, minute=8, second=00):
                    cell.fill = orange
                    print('orange')
                else: 
                    cell.fill = red
